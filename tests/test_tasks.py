...
class CliTests(unittest.TestCase):
    ...
    def test_cli_remaining_effort_tracks_status_transitions(self):
        for status, remaining in (("todo", 9), ("doing", 9), ("done", 0)):
            with self.subTest(status=status):
                result = self.invoke(json.dumps([
                    {"id": "active", "status": status, "points": 9},
                    {"id": "shipped", "status": "done", "points": 20},
                ]))
                self.assertEqual(result.returncode, 0, result.stderr)
                ...
                counts[status] += 1
                self.assertEqual(json.loads(result.stdout), {
                    "total": 2, "counts": counts, "remaining_points": remaining,
                })
                self.assertEqual(result.stderr, "")
